...
@app.route('/duration-milliseconds-count', methods=['POST'])
def send_anomalies_duration_millisecond_count():
    data = request.json
    ...
    try:
        cursor = connection.cursor()  # Use cursor() method instead of a context manager with mysql.connector
        sql = "INSERT INTO duration_millisecond_count (timestamp, value, is_anomaly) VALUES (%s, %s, %s)"
